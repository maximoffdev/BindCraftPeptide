...
def repredict_and_score_structure(sequence, basis_design_name, design_paths, trajectory_pdb, length, helicity_value,
                  seed, prediction_models, binder_chain, filters, design_labels, target_settings, 
                  advanced_settings, advanced_file, settings_file, filters_file, stats_csv, 
                  failure_csv, final_csv, complex_prediction_model, binder_prediction_model, 
                  is_mpnn_model=False, mpnn_n=None, design_path_key="Repredicted"):
    ...
    #   - Returns statistics dict and pass/fail boolean
    complex_statistics, pass_af2_filters = predict_binder_complex(complex_prediction_model,
                                                                    sequence, basis_design_name,
                                                                    target_settings["starting_pdb"], target_settings["chains"],
                                                                    length, trajectory_pdb, prediction_models, advanced_settings,
                                                                    filters, design_paths, failure_csv, design_path_key=design_path_key)
    ...
    # For each AF2 model weight used in prediction, calculate detailed structural properties
    for model_num in prediction_models:
        design_pdb = os.path.join(design_paths[design_path_key], f"{basis_design_name}_model{model_num+1}.pdb")
        design_relaxed = os.path.join(design_paths[f"{design_path_key}/Relaxed"], f"{basis_design_name}_model{model_num+1}.pdb")

        if model_num + 1 not in complex_statistics:
            complex_statistics[model_num + 1] = {}
    ...
    # predict_binder_alone: Runs AF2 in hallucination mode (no template) with just binder sequence
    #   Returns structure quality metrics (pLDDT, pTM, pAE)
    binder_statistics = predict_binder_alone(binder_prediction_model, sequence, basis_design_name, length,
                                            trajectory_pdb, binder_chain, prediction_models, advanced_settings, 
                                            design_paths, design_path_key=design_path_key)

    # === CALCULATE BINDER-ALONE RMSD TO TRAJECTORY ===
    # Check how much the binder structure changes when target is removed
    # Large RMSD indicates binder is not stable alone (requires target to fold)
    for model_num in prediction_models:
        binder_pdb = os.path.join(design_paths[f"{design_path_key}/Binder"], f"{basis_design_name}_model{model_num+1}.pdb")
        ...
        # Ensure dict exists even if predict_binder_alone skipped due to existing PDB
        if model_num + 1 not in binder_statistics:
            binder_statistics[model_num + 1] = {}
        ...
        if rmsd_binder is not None:
            ...
            raise ValueError(f"RMSD calculation failed for {basis_design_name} model {model_num+1}")
    ...
    #   - Avoids cysteines if specified (can form unwanted disulfides)
    #   - Checks for unusual amino acid patterns
    clash_val = complex_averages.get('Relaxed_Clashes', None)
    if clash_val is None:
        clash_val = 0
    seq_notes = validate_design_sequence(sequence, clash_val, advanced_settings)
    ...
    # Start with basic design metadata
    data = [basis_design_name, advanced_settings["design_algorithm"], length, seed, helicity_value, target_settings["target_hotspot_residues"], sequence, interface_residues, "", ""]
    ...
    # === SELECT BEST MODEL BY PLDDT AND STORE ===
    best_model_pdb = None
    best_model_number = None
    best_plddt = None
    for model_num in prediction_models:
        model_stats = complex_statistics.get(model_num + 1, {})
        model_plddt = model_stats.get('pLDDT', None)
        if model_plddt is None:
            continue
        if best_plddt is None or model_plddt > best_plddt:
            best_plddt = model_plddt
            best_model_number = model_num + 1

    if best_model_number is not None:
        best_model_pdb = os.path.join(design_paths[f"{design_path_key}/Relaxed"], f"{basis_design_name}_model{best_model_number}.pdb")
        if os.path.exists(best_model_pdb):
            target_best = os.path.join(design_paths.get(f"{design_path_key}/Best", design_paths[f"{design_path_key}/Relaxed"]), os.path.basename(best_model_pdb))
            os.makedirs(os.path.dirname(target_best), exist_ok=True)
            shutil.copy(best_model_pdb, target_best)

    # === STORE FINAL ROW WITHOUT FILTERING ===
    final_data = [''] + data
    insert_data(final_csv, final_data)
    ...
    # if filter_conditions == True:
    #     # === DESIGN PASSED ALL FILTERS - ACCEPT IT ===
    #     print(basis_design_name+" passed all filters")
    ...
    return None, stats_csv, failure_csv, final_csv
