...
install_dir=$(pwd)
CONDA_BASE=/home/sc.uni-leipzig.de/nw99ixuq/.conda || { echo -e "Error: conda is not installed or cannot be initialised."; exit 1; }
...
conda env list | grep -w 'BindCraft' >/dev/null 2>&1 || { echo -e "Errorc: Conda environment 'BindCraft' does not exist after creation."; exit 1; }

# Load newly created BindCraft environment
echo -e "Loading BindCraft environment\n"
source /software/easybuild/el8/amd_zen1/all/Anaconda3/2024.02-1/bin/activate /home/sc.uni-leipzig.de/nw99ixuq/.conda/envs/BindCraft || { echo -e "Error: Failed to activate the BindCraft environment."; exit 1; }
...
source_params_dir="/home/sc.uni-leipzig.de/nw99ixuq/alphamask_setup/params"

# Check if source params directory exists and contains the required files
if [ -d "${source_params_dir}" ] && [ -f "${source_params_dir}/params_model_5_ptm.npz" ]; then
    echo -e "Found existing AlphaFold2 weights, creating symbolic links\n"
    mkdir -p "${params_dir}" || { echo -e "Error: Failed to create weights directory"; exit 1; }
    
    # Create symbolic links for all parameter files
    for param_file in "${source_params_dir}"/params_model_*.npz; do
        ln -s "${param_file}" "${params_dir}/$(basename ${param_file})" || { echo -e "Error: Failed to create symbolic link for $(basename ${param_file})"; exit 1; }
    done
    
    # Also link the LICENSE file if it exists
    if [ -f "${source_params_dir}/LICENSE" ]; then
        ln -s "${source_params_dir}/LICENSE" "${params_dir}/LICENSE" || { echo -e "Warning: Failed to create symbolic link for LICENSE file"; }
    fi
else
    # Original download logic if source params don't exist
    mkdir -p "${params_dir}" || { echo -e "Error: Failed to create weights directory"; exit 1; }
